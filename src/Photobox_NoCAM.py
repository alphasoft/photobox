...
class Camera:
   ...
   def takeSnapshot(self,pl):
      if self.snapshot_counter>0: # more snaps to be shot
        ...
        Log.trace(avg.Logger.APP, "Snapshot counter from %i to %i" % (self.snapshot_counter+1,self.snapshot_counter))
      ...
      if self.snapshot_counter>0: # more snaps ? nextpic : end procedure
        ...
        ioBoard.lightsOff()
        Log.trace(avg.Logger.APP, "cleanup after snapshots - actionDone has been sent.")
        control.event("actionDone")  # All snapshots saved, now call scr.prepSnapPreview()

   # snapshot-Saver, called from above routine
   def saveSnapshot(self):
      photoFile = photoPath+"snapshot_0"+str(numSnapshots-self.snapshot_counter)+".bmp"
      ...
      if ioBoard.coinsInserted:
        if not ioBoard.coinsTaken:
           ioBoard.takeCash()  # now coins are Taken  # Münzen eingenommen
...
if __name__=="__main__":
    ...
    ioBoard = IODevice(pl,"/dev/parport0")  # Relais-Steuerkarte (avg.ParPort-Instance wird angelegt)
