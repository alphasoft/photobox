...
ourCamera = None
#simulation = False  - jetzt Eigenvariable von ioBoard
scrSaver = 0
switchOnOff = None
...
def onMouseOver():
    ...
    if scr.currScreen=="snapshot_select":  # balken existiert nur auf diesem screen
       ...
       pl.getElementByID('balken').opacity = 0.6
...
class Screen:
  ...
  listOfNodes = ("screensaver","motivauswahl","muenzeinwurf",
                 "camera_screen","snapshot_select","photomontage","waitscreen","alertscreen")
  selectionScreens = ("motivauswahl","snapshot_select","photomontage")
  ...
  def prepMuenzEinwurf(self): # Vorbereitung des Muenzeinwurfs
     ...
     self.gotoScreen("muenzeinwurf")
     # Großes Vorschaubild mit Ellipsenausschnitt zur Positionierung anzeigen
     self.pl.getElementByID('selmotiv_container').y = 310
  ...
  def prepAlertScreen(self):
      self.pl.showCursor(1)
      self.gotoScreen("alertscreen")
...
class Camera:
   ...
   def takeSnapshot(self,pl):
      ...
      if self.snapshot_counter==0:  # last picture taken ?
...
class IODevice:
   ...
   def __init__(self,player,dev="/dev/parport0"):  # connected via parallel port
       ...
       parPort = avg.ParPort()
       parPort.init("") ### makes dev parameter obsolete
       ...
       self.detectValue = False    # react upon "low" signal (~ACK)
       # VARIABLEN
       self.activity = ""
   ...
   def checkCoinInput(self):
       ...
       if self.activity=="ret":
          ...
          moneyfile = open(logPath+"amount_paid",'w')
          moneyfile.write(time.asctime())
          moneyfile.close()
          if not self.coinsTaken:
             self.coinsInserted=True  # Flag to indicate coins present => we can cash them later
          ...
          zoom.start()   # start zooming, send event "actionDone" to control when finished
       elif self.activity=="":
          pass

   def geldCheck(self):  # check 'money-present' signal, get ready for cashing or rejecting
       if self.simulate:  # not necessary in simulation mode
          return
       if self.coinsTaken:     ### REIHENFOLGE ?
          scr.checkCoinsReturn.stop()
          return  #  Rueckgabe nicht mehr moeglich
       # "else" : Rueckgabe noch moeglich :
       status = self.Card.getStatusLine(avg.STATUS_ACK)
       if not status:
   ...
   def lightsOff(self):  # set lights off and reset status flags
       self.Card.clearDataLines(255)  # 255 ==> all Bits
       self.activity = ""
       self.simulate = False
...
def insertFace(face, motivBgrd, composition):   # alle Param. Dateinamen mit Pfaden
    ...
    print "insertFace() von ? aufgerufen !"
...
def printPostcard():
    global lpCheck, switchOnOff, photoPath, selSnap, selMotiv, postcardFile
    print "deleting old postcard (which has been saved)..."
    os.remove(postcardFile)
    scr.gotoScreen('waitscreen')
    pl.getElementByID('huepfender_fussball').play()
    startBlinken = pl.setTimeout(16500, blinken)  ### unelegant!  Wird ersetzt! ###
    endBlinken  =  pl.setTimeout(25500, lambda:control.event("time"))
    try:
    ...
    ###  wait for compose to finish the job (new postcard saved to disk)
    while not os.path.exists(postcardFile):
      pass
    os.system(scriptPath+"do_print.sh "+postcardFile+" &")  # now print!
    if os.path.exists(postcardFile):  # save all printed output for persistence
      os.system(scriptPath+"archive_png.sh "+postcardFile+" &")
    if os.path.exists(logPath+"amount_paid"):
      os.remove(logPath+"amount_paid")
...
def build_fsm(scr, ourCamera):
    ...
    states=('ScrSav','MotivSel','CoinOp','ZoomingUp','CamReady',
            'TakePics','SnapSel','Preview','WaitForPrint','ShowAlert')
    for st in states:
    ...
    control.add_transition("Preview","alert", action=scr.prepAlertScreen, new_state="ShowAlert")
